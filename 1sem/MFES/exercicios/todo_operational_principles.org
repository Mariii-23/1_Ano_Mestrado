...
pred op3 { //?
   ...
   all t: Task | always add[t] implies after t in Pending'
}
...
pred op8 { //??
    ...
    //all t : Task | always t in Done implies after delete[t]
    always ((Task in Done and Task not in Pending) implies  always (all t:Done | delete[t] ))
}
