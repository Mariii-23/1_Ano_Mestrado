...
public:
    ...
    void mprocess(int num[], int tam) {
        ...
        for(int i=0; i<tam; i++) {
            if (num[i]>2) {  // if was not removed
                if (isPrime(num[i],myPrimes,nPrimes)) totalPrimes++;
                else num[i]=0;  // remove from list
            }
        }
    }
    
    // end of processing (show statistics from this filter)
    void end() {
        double timef = 0;//MPI_Wtime();
        std::cout << /*"Demorou " << (timef-timei)/1000 << "s " << */totalPrimes << " primes\n";
    }
};
